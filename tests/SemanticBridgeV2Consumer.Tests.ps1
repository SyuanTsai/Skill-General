...
function New-SyntheticResumePlan {
    ...
    $plan = [ordered]@{
        ...
        authority = [ordered]@{
            revision = [string]$script:AuthorityConfig.authority.commit
            archivePath = Join-Path $runRoot 'authority.zip'
            archiveSha256 = [string]$script:AuthorityConfig.authority.archiveSha256
...
function Invoke-SyntheticResume {
    ...
    return [pscustomobject]@{
        ...
        $script:AuthorityConfig = Get-Content -LiteralPath (Join-Path $script:RepositoryRoot 'config/standard-v1.json') -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 20
    }
...
'@, [Text.UTF8Encoding]::new($false))
    ...
    It 'prepares, signs, resumes, verifies, and rejects replay through the exact Validate.ps1 entrypoint' -Tag 'SemanticBridgeV2ConsumerE2E' {
        ...
        $plan.authority.revision | Should -BeExactly ([string]$script:AuthorityConfig.authority.commit)
