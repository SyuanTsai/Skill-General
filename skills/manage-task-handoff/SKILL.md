...
After the user selects, combines, or rejects branches, conditionally write the common decision and verify readback first. Then append each Branch Outcome (`Selected`, `Partially Selected`, or `Superseded`) with reason and provenance; archive selected and unused branches immediately unless the user expressly keeps one exploring. For every archived branch, bind its read-back Archived revision before conditionally removing its exact Branch ID from the common Active index with a stable operation ID. Re-read Lifecycle before a removal retry and verify Lifecycle plus index afterward. If concurrent continuation restored the branch, cancel the stale removal and conditionally re-add a missing ID before reporting success. A failed common write or readback leaves branches Active and retryable. A later branch archive, index, compensation, or readback failure retains the exact record and operation IDs as partial reconciliation, and the common record stays protected while an Archived branch remains indexed. `Conflict` and `Superseded` do not expand Lifecycle states. Implementation ready for review uses `Awaiting Review`; there is no `Completed` Work State.
