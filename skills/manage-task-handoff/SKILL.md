...
4. Before creating any missing peer branch, including B when A already has a branch record, conditionally persist a pending reconciliation marker in common Current or equivalent mapped metadata and read it back. For an existing A, bind that marker to A's exact Branch ID and revision, a lossless snapshot of A's Current and Source at the fork, the independently verified shared Current and Source baseline, the intended B ID, the fork point, the verified Active-branch index, and stable B-create and index-update Operation IDs. When this marker replaces common Current, it must also retain that exact prior common Current so clearing can restore it; otherwise use separate mapped metadata. If A advances after the marker is durable, reconciliation verifies the same Task Key and Branch ID, leaves the newer A untouched, and creates B from the persisted shared baseline; it does not need historical-revision lookup or copy A's post-fork work. Clear the marker only after B and its index entry are read back and the retained common value is restored and verified. On the first fork of an A conversation that still has only a common record, the pending payload must additionally preserve A's pre-fork Current and Source losslessly with the verified shared Current and Source baseline. Create A from that snapshot and B from the confirmed baseline, not from A's unselected candidate conclusion. Both branches share one stable fork point. Read back A, B, their common index, and the confirmed common Current and Source before clearing pending; A-only Source must not remain in common. Follow the exact partial-failure sequence in [Handoff operations](references/handoff-operations.md).
